describe("Testing styleset modifications", function() {
  ...
  it("changes all colors when the dominant is changed", function() {
    ...
    cy.get(":nth-child(3) > ._container > :nth-child(3) > ._color-range")
      .invoke("val", 10)
      .trigger("input");
    cy.get("#_color > :nth-child(3)").should(
      "have.css",
      "background-color",
      "rgb(212, 160, 150)"
    );
    cy.get("#_color > :nth-child(4)").should(
      "have.css",
      "background-color",
      "rgb(110, 196, 180)"
    );
    cy.get("#_color > :nth-child(5)").should(
      "have.css",
      "background-color",
      "rgb(150, 210, 212)"
    );
  ...
  it("generates proper color subcombinations", function() {
    cy.get(
      "#_color > :nth-child(3) > ._container .sub-combination:nth-of-type(9)"
    ).should("have.css", "background-color", "rgb(240, 210, 204)");
  ...
  it("changes colors on the selector panel", function() {
    cy.get("#_selector ._selector-box:first div div").should(
      "have.css",
      "background-color",
      "rgb(161, 93, 79)"
    );
    ...
    cy.get(
      '#_selector li:nth-of-type(2) :nth-child(2) > div[contenteditable="true"] > div'
    ).should("have.css", "background-color", "rgb(58, 98, 100)");
  });
  it("changes the DOM when the dominant is changed", function() {
    cy.get("#rf-content-container").should(
      "have.css",
      "background-color",
      "rgb(192, 236, 228)"
    );
