...
const getParameter = function(coordinates, styleSet, colorSet) {
  if (coordinates.includes('"type":"color"')) {
    ...
    if (typeof coordinate === "string") {
      ...
      return colorSet.combinationCollection[coordinate].subCombination[
        subCoordinate
      ].hex;
    }
...
const getColorCSSVariables = function(colorSet) {
  ...
  for (let i = 0; i < colorSet.dominantSubCollection.length; i++) {
    colorVariable += `    --dominant${i}: ${colorSet.dominantSubCollection[i].hex};\n`;
  }
  for (let i = 0; i < colorSet.combinationCollection.length; i++) {
    colorVariable += `    --color${i}: ${colorSet.combinationCollection[i].hex};\n`;
    for (
      let j = 0;
      j < colorSet.combinationCollection[i].subCombination.length;
      j++
    ) {
...
const getRatioCSSVariables = function(ratioCollection) {
  let ratioVariables = "";
  for (const [key, value] of Object.entries(ratioCollection)) {
    ...
    if (key === "html") {
      ...
      totalRatio =
        parseInt(value["line-height"]) +
        parseInt(value["margin-top"]) +
        parseInt(value["margin-bottom"]);
    }
    ratioVariables += `    --ratio-${key}: ${totalRatio}rem;\n`;
  }
...
export const generateCSS = function(styleSet) {
  ...
  for (const [selector, properties] of Object.entries(customCSS)) {
    ...
    for (const [property, value] of Object.entries(properties)) {
      if (value && value !== "0rem") {
        propertiesString += `    ${property}: ${getParameter(
...
${getRatioCSSVariables(styleSet.ratioCollection)}
}
html{
${htmlProperties}
}
#rf-content-container {
    font-family: ${makeFontFamilyName(
      styleSet.fontFamilyMain
    )};${bodyProperties}
}
${headerTags} {
