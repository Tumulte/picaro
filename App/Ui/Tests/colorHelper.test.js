...
describe('The color type stays the same if it is already the correct one', () => {
	it('return the RGB if it is already a RGB', () => {
		var color = { red: 23, green: 189, blue: 38 };
		var rgb = colorUtils.hexToRgb(color).getValueCollection();
...
describe('Converts string to object', () => {
	it('return the hsl', () => {
		expect(colorUtils.getValueCollection('hsl(100,20,340)')).toEqual({
			hue: 100,
			saturation: 20,
			light: 340,
		});
...
describe('Converts object to string', () => {
	...
	it('returns the string of an RGB object', () => {
		expect(colorUtils.getString({ red: 233, green: 109, blue: 29 })).toEqual('rgb(233,109,29)');
	});
	it('returns the string of an HSL object', () => {
		expect(colorUtils.getString({ hue: 278, light: 90, saturation: 29 })).toEqual('hsl(278,29%,90%)');
	});
	it('returns the string of an Hex object', () => {
		expect(colorUtils.getString({ hexred: 'E3', hexgreen: '09', hexblue: 'AA' })).toEqual('#E309AA');
	});
});
