...
wrapper.vm.localFontCollection = [
    {family: "InriaSans-Bold.otf"}, {family: "EBGaramond-InitialsF1.otf"}, {family: "Condensed.otf"}];
wrapper.vm.googleFontCollection = [{family: "google"}];

const genericFonts = [{family: "Arial"},
    {family: "Verdana"},
    {family: "Helvetica"},
    {family: "Tahoma"},
    {family: "Trebuchet MS"},
...
describe("Handles the saveStyleSet and the fonts", () => {
    it("changes the styleset when another is selected", async () => {
        expect(wrapper.vm.fontCollection).toStrictEqual([...genericFonts,
            {family: "InriaSans-Bold.otf"}, {family: "EBGaramond-InitialsF1.otf"}, {family: "Condensed.otf"}]);
        ...
        expect(wrapper.vm.fontCollection).toStrictEqual([...genericFonts, {family: "google"}]);

        expect(wrapper.vm.$store.getters.colorSet.colorCollection.dominant).toBe("#F27F38");
        expect(wrapper.vm.$store.getters.colorSet.colorCollection.combinationCollection[0].hex).toBe("#F25E36");
        expect(document.getElementById("rf-live-styles").innerHTML).toContain("@import url(\"https://fonts.googleapis.com/css?family=test2.otf&display=swap\");");
    ...
    it("changes the font of the DOM", async () => {
        ...
        expect(computedCss.getPropertyValue("font-family")).toBe("EBGAramond-InitialsF1");
