...
var updateTagString = function(tagCollection, model, property) {
	...
	methods: {
		detectKeyPress: function(e) {
			if (e.keyCode === keycode.down) {
				var withinList = this.matchingTagCollection.length > this.currentMatchingTagIndex;
				if (withinList) {
					this.currentMatchingTagIndex =
						this.currentMatchingTagIndex === false ? 1 : this.currentMatchingTagIndex + 1;
					this.setExistingTag();
				}
			...
			} else if (e.keyCode === keycode.enter || e.keyCode === keycode.comma || e.keyCode === keycode.space) {
				e.preventDefault();
				e.stopPropagation();
		...
		setExistingTag: function() {
			if (this.currentMatchingTagIndex - 1 >= 0) {
				this.selectedExistingTag = this.matchingTagCollection[this.currentMatchingTagIndex - 1];
			} else {
		...
		filterTag: function(type) {
			var tagPropertyCollection = [];
			for (var i = 0; i < this.matchingTagCollection; i++) {
		...
		removeTag: function(index) {
			...
			this.tagStringCollection = updateTagString(this.currentTagCollection, this.rfModel, 'name');
		},
		removeExistingTag: function(index) {
			this.currentExistingTagCollection.splice(index, 1);
			this.existingTagStringCollection = updateTagString(this.currentExistingTagCollection, this.rfModel, 'id');
		},
		populateTags: function() {
			if (this.rfData && this.rfData.tags) {
				var tagsData = [];
				for (var tag in this.rfData.tags) {
					tagsData.push(this.rfData.tags[tag].id);
					this.currentExistingTagCollection.push(this.rfData.tags[tag]);
				}
				this.existingTagStringCollection = this.rfModel + '__' + tagsData.join(',');
			}
		},
	},
	watch: {
