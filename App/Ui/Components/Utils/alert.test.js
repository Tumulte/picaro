...
describe("modelPanel", () => {
    it("adds an alert to the alert list", async () => {
        wrapper.vm.$store.dispatch("addAlert",
            {text: "this is a warning message", type: "warning", timeOut: 100}
        );
        wrapper.vm.$store.dispatch("addAlert",
        ...
        expect(wrapper.html().includes("div class=\"v-alert__content\">this is a warning error")).toBe(false);
        expect(wrapper.html().includes("div class=\"v-alert__content\">isn't it anonther")).toBe(false);
