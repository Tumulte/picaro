  ...
  div
    ...
    div(class="px-5 py-5")
      v-form(id="_admin-form" method="POST" @submit.prevent="saveConfig($event)")
        ...
        v-text-field(name="name" v-model="settings.applicationName" label="Picaro Name" dense outlined)
        v-text-field(name="styleSet" v-model="styleSet.id" dense)
        v-text-field(name="language" v-model="settings.language" label="Picaro Language" dense outlined)
        v-text-field(name="title" v-model="settings.title" label="Picaro Title" dense outlined)
...
export default {
  ...
  methods: {
    createNewModel() {
      axios
          .post(`/api/newmodel/${this.newModelName}`)
          .then(() => {
    ...
    sendAdminForm: function () {
      this.$nextTick(() => {
        axios
            .put("/settingapi/settings/", this.settings)
            .then(() => {
