...
const recursiveUpdateParentCollection = function (destinationNode, movingNode, index) {
  ...
  if (Object.keys(movingNode.children).length > 0) {
    for (const child in movingNode.children) {
...
const generateStructureFromFileList = function (views) {
  ...
  const parsedFiles = views ? JSON.parse(views) : [];
  let indexOffset = 0;
...
export default {
  ...
  methods: {
    ...
    targetIsNotAlreadyParent: function (node) {
      ...
      return !selectedDestinationFile.children.hasOwnProperty.call(this.movingNode.name);
    },
    canMoveAtDestination: function (node, index) {
      const targetIsNotAlreadyParent = this.targetIsNotAlreadyParent(node, index);
      const targetIsNotItself = this.getSubNode(node) !== this.movingNode.data;
    ...
    changeSubsequentNumbering: function (destinationBranch, newPosition) {
      ...
      for (const nodeKey in destinationBranch) {
    ...
    saveNavEntry: function () {
      this.warningMessage = {
        text: "Are you sure you want to saveStyleSet that navigation ?",
        type: "warning",
        callback: () => {
