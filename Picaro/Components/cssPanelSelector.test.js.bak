...
const wrapper = mount(selectorPanel, {
  vuetify,
  store,
  propsData: {
    miniVariant: false
...
const colorWrapper = mount(cssPanelColor, {
  vuetify,
  store,
  propsData: {
    miniVariant: false
...
describe("The selector panel handles the CSS edition of the app", () => {
  it("it creates selectors", async () => {
    ...
    expect(wrapper.vm.$store.getters.alertCollection[0].text).toContain(
      "valid CSS"
    );
  ...
  it("cancels edition if the selector already exists", () => {
    ...
    expect(wrapper.vm.$store.getters.alertCollection[1].text).toContain(
      "exists"
    );
    expect(wrapper.vm.selectorCollection).toStrictEqual({
      ".css-test": {},
  ...
  it("creates properties for selectors", async () => {
    wrapper
      .findAll("[data-jest='selector-accordion']")
      .at(2)
      .trigger("click");
    await wrapper.vm.$nextTick();
    ...
    await wrapper.vm.$nextTick();
    expect(wrapper.vm.$store.getters.alertCollection[2].text).toContain(
      "valid CSS property"
    );
    ...
    propertyInput.setValue("color");
    propertyInput.trigger("keyup.enter");
    expect(wrapper.vm.$store.getters.alertCollection[3].text).toContain(
      "The property color already exists "
    );
  ...
  it("adds the proper color to the selector", async () => {
    ...
    await wrapper.vm.$nextTick();
    let colorSquare = colorWrapper
      .findAll('[data-jest="sub-combination-square"]')
      .at(14);
